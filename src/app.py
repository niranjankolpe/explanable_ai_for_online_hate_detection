...
# If you load a 3rd model, it deletes the oldest one from RAM. This prevents 8GB laptops from crashing!
...
# Dynamically check which models are available on disk without loading them into memory
...
with tab2:
    ...
    exp_model = st.selectbox("Select Model to Explain", available_models, key="exp_model")
...
with tab3:
    ...
    rag_model = st.selectbox("Select Classifier", available_models, key="rag_model")
    rag_llm   = st.selectbox("Select Explainer LLM", ["Gemini", "Ollama (Llama 3.1)"], key="rag_llm")
...
with tab5:
    ...
    crawl_model = st.selectbox("Select Model for Analysis", available_models, key="crawl_model")
...
with tab7:
    ...
    stream_model = st.selectbox("Select Model for Live Analysis", available_models, key="stream_model")
...
with tab8:
    ...
    agent_url = st.text_input("Enter Starting URL for Audit", value="https://example.com", key="agent_url")
    ...
    if st.button("Run Agentic Audit"):
        if not agent_url.strip():
            ...
            st.error("Google API Key is required to run the agentic workflow using Gemini.")
        else:
