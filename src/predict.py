...
def load_model(model_type: str, subtask: str = "a"):
    ...
    if model_type == "baseline":
        ...
        vectorizer = joblib.load(
            f"models/baseline_{subtask}/tfidf_vectorizer.pkl")
    ...
    if model_type == "lstm":
        ...
        model.load_state_dict(
            torch.load(
                f"models/lstm_{subtask}/lstm_model.pt",
                map_location="cpu"))
        model.eval()
        return model, vocab
    ...
    if model_type == "llama":
        ...
        adapter_path = "models/llama3.2_3b_lora_hate_speech"
...
def predict_proba(
    ...
    if model_type == "baseline":
        ...
        if subtask is not None:
            # sklearn sorts model.classes_ alphabetically; reorder to match
            # params.yaml label order
            target_labels = _params["subtasks"][subtask]["labels"]
            col_order = [list(model.classes_).index(lbl)
    ...
    if model_type == "lstm":
        ...
        for text in texts:
            ...
            seq = [vocab.get(t, vocab["<UNK>"])
                   for t in text.split()] if text else [vocab["<UNK>"]]
