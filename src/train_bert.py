...
def train(subtask: str) -> None:
    ...
    df["tweet"] = df["tweet"].apply(
        preprocess_common)   # consistent preprocessing
    texts = df["tweet"].tolist()
    labels = [label2idx[lbl] for lbl in df[cfg["column"]]]
    ...
    with mlflow.start_run():
        mlflow.log_params(
            {**p, "subtask": subtask, "preprocessing": "preprocess_common"})

        for epoch in range(p["epochs"]):
            print(
                f"  Epoch {epoch+1}/{p['epochs']} started at {time.strftime('%H:%M:%S')}")
            model.train()
            total_loss = 0.0
            ...
            for i, batch in enumerate(train_dl):
                ...
                if (i + 1) % 50 == 0:
                    print(
                        f"    Batch {i+1}/{len(train_dl)} | Loss: {loss.item():.4f} | {time.strftime('%H:%M:%S')}")
            ...
            mlflow.log_metrics(
                {"loss": avg_loss, "val_acc": val_acc, "val_f1_weighted": val_f1}, step=epoch)

        mlflow.log_metrics(
            {"final_val_acc": val_acc, "final_val_f1_weighted": val_f1})
